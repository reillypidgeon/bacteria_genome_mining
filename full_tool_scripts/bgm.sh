...
bash ${download_scripts}/01_genome_extraction.sh "${taxa}"

bash ${download_scripts}/02_genome_download.sh "${results_dir}/accessions_out/genomes_*_r232.tsv"

bash ${download_scripts}/03_genome_preparation.sh "${genomes_dir}"

bash ${analysis_scripts}/04_pyrodigal_annotations.sh "${genomes_dir}/*.fna"

bash ${analysis_scripts}/05_mmsequs2_search.sh --min-seq-id ${min_seq_id} --min_coverage ${min_coverage} ${query_fasta} ${subject_fastas}
