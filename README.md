...
```
**02_genome_download** <br>
Specify at least one tab-separated file (produced from `01_genome_extraction.sh`) containing a column with NCBI accessions and a column with corresponding NCBI assemblies. Note that spaces in species names are replaced with underscores to avoid errors with file paths. If genomes have already been downloaded and processed, the download step will be skipped.
```
...
bash 02_genome_download.sh "../results/accessions_out/genomes_g__Enterocloster_r232.tsv" "../results/accessions_out/genomes_s__Hungatella_hathewayi_r232.tsv"
bash 02_genome_download.sh "../results/accessions_out/genomes_*_r232.tsv"
```
**03_genome_preparation** <br>
Specify a directory containing the genome FASTA files (`.fna.gz` that were downloaded from the NCBI) to unzip and annotate with the proper accession code in the FASTA header.
```
# Unzip genome FASTA files and add accessions to FASTA headers for all FASTA files (.fna) in the genomes directory
...
<br>
**04_pyrodigal_annotations** <br>
Specify at least one genome FASTA file (`.fna`) or a directory containing genome FASTA files (`.fna`). If annotations already exist for a given genome, the annotation step will be skipped.
```
...
```
**05_mmseqs2_search** <br>
Specify a query FASTA file (`.faa` or `.fna`) and at least one subject FASTA file (`.faa` or `.fna`, generated by `04_pyrodigal_annotations.sh`). Alternatively, you can specify a query FASTA file (`.faa` or `.fna`) and a directory containing FASTA files (`.faa` or `.fna`). <br>
Optional arguments **must be placed before** the required query FASTA and subject FASTA(s). These include:
- `--out-dir`: Output directory for the mmseqs2 search (STRING | Default: `${project_dir}/results/mmseqs2_out/mmseqs2_search_${time_code}`)
  - Adds a unique time code formatted according to the YYYYMMDDHHMM
- `--min-seq-id`: Minimum sequence identity cutoff (FLOAT between 0-1 | Default: `0.5`)
- `--min-coverage`: Minimum sequence coverage cutoff (FLOAT between 0-1 | Default: `0.5`)
...
### Results
The following directories will be created within the `bacteria_genome_mining` directory:
- `genomes/`: Main directory containing all genome FASTA files (`.fna`) for specified taxa and from other taxa from previous runs
  - Avoids duplicate downloads in case there is overlap between taxonomic levels
- `results/`: Main directory containing
